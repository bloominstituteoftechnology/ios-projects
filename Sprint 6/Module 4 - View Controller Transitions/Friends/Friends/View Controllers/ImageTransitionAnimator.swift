    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        if toggle {
            ...
            UIView.animate(withDuration: duration, animations: {
                ...
                destinationName.alpha = 1.0
                //destinationTitle.alpha = 1.0
            ...
            guard let fromVC = transitionContext.viewController(forKey: .from) as? ViewController,
                let toTVC = transitionContext.viewController(forKey: .to) as? TableViewController,
                let toView = transitionContext.view(forKey: .to) else {
                    return
            }
            ...
            let toViewEndFrame = transitionContext.finalFrame(for: toTVC)
            toView.frame = toViewEndFrame
            toView.alpha = 0.5
            // ^^^^^^^^^^^^^ The destination view is immediately on screen
            // so we set the alpha to zero, so the user can't see it...
            ...
            let sourceName = fromVC.friendName!
            //let sourceTitle = fromVC.friendTitle!
            let sourceImage = fromVC.friendImage!
            sourceName.alpha = 0.5
            //sourceTitle.alpha = 0.5
            sourceImage.alpha = 0.5
            
            
            let destinationName = toTVC.nameLabel!
            //let destinationTitle = toTVC.nameTitle!
            let destinationImage = toTVC.imageView!
            destinationName.alpha = 0.0
            //destinationTitle.alpha = 0.0
            destinationImage.alpha = 0.0
